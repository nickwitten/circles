...
class TestMembersCompleted(CreateLearningModelsMixin, TestCase):
    ...
    def test_GET_members_completed(self):
    ...
    def test_POST_members_completed(self):
        theme = self.theme1_1
        # profile = members_models.Profile.objects.get(pk=1)
        ...
        # data = json.loads(response.content)
        # self.assertEqual(response.status_code, 200)
        # self.assertEqual(data['profiles'], [[str(profile), 1]])
